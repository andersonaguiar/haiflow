...
> **Why not the Claude API?** Claude Code includes tool use, file access, git integration, and your custom skills out of the box. Haiflow lets you automate all of that via HTTP without paying per-token API costs. Use cron, webhooks, or any automation tool to drive it.
...
If you need to access haiflow remotely (from webhooks, remote clients, etc.), see [DEPLOYMENT.md](DEPLOYMENT.md) for a guide on setting up Cloudflare Zero Trust Access — adds an identity layer so a stolen API key alone isn't enough.

## Documentation

Full developer documentation lives in [`docs/`](docs/), a searchable [Mintlify](https://mintlify.com) site covering the quickstart, every endpoint (with an interactive playground generated from `docs/openapi.json`), the MCP server, pipelines, deployment, and security. Preview it locally:
...
`integrations/haiflow-mcp/` is an MCP server that exposes haiflow as tools (`haiflow_run`, `haiflow_start_session`, `haiflow_trigger`, `haiflow_get_response`, `haiflow_stop_session`, `haiflow_status`, `haiflow_doctor`), so any MCP-capable agent (Claude Desktop, Cursor, Cline, another Claude Code) can orchestrate Claude Code through haiflow. See `integrations/haiflow-mcp/README.md` for wiring. Inside Claude Code, the `haiflow` skill teaches an agent to drive the HTTP API directly.
...
```json
{
  "topics": {
    "review.done": {
      ...
      "webhooks": [
        {
          "url": "https://hooks.example.com/review-done",
...
Inject events from outside (scripts, webhooks):
...
├── examples/
│   └── chained-calc/         # Chained calc pipeline example
├── assets/
...
| `bun run doctor` | Full health check (server, sessions, pipeline) |
| `bun test` | Run tests |
